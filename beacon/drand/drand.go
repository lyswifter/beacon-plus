...
import (
	...
	log "github.com/lyswifter/beacon-plus/log"
...
func NewDrandBeacon(genesisTs, interval uint64, ps *pubsub.PubSub, config dtypes.DrandConfig) (*DrandBeacon, error) {
	...
	dlogger := dlog.NewKitLoggerFrom(kzap.NewZapSugarLogger(
		logg.SugaredLogger.Desugar(), zapcore.InfoLevel))
	...
	if ps != nil {
		...
		logg.Info("drand beacon without pubsub")
	}

	client, err := dclient.Wrap(clients, opts...)
	if err != nil {
		return nil, xerrors.Errorf("creating drand client: %s", err)
	}
...
func (db *DrandBeacon) Entry(ctx context.Context, round uint64) <-chan beacon.Response {
	...
	go func() {
		...
		log.Infof("start fetching randomness for round: %d", round)
		...
		log.Infof("done fetching randomness for round: %d, took: %s", round, build.Clock.Since(start).String())
		out <- br
		close(out)
	}()
